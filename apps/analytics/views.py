...
from django.db.models.functions import Extract, TruncDay, TruncMonth, TruncWeek
...
def analytics_dashboard(request):
    ...
    overdue_tasks = Task.objects.filter(
        due_date__lt=timezone.now(), status__in=[Task.TODO, Task.IN_PROGRESS]
    ).count()

    # User stats
    total_users = User.objects.count()
    recent_reports = Report.objects.filter(user=request.user).order_by("-created_at")[
        :5
    ]

    # Get some recent analytics snapshots if available
    recent_snapshots = AnalyticsSnapshot.objects.filter(project=None).order_by(
        "-timestamp"
    )[:7]
...
def project_detail_analytics(request, project_id):
    ...
    completion_trend = (
        tasks.filter(status=Task.COMPLETED, completed_at__gte=thirty_days_ago)
    ...
    snapshots = AnalyticsSnapshot.objects.filter(project=project).order_by(
        "-timestamp"
    )[:14]
...
def productivity_analytics(request):
    ...
    for user in users:
        ...
        completed_tasks = Task.objects.filter(
            assigned_to=user, status=Task.COMPLETED
        ).count()
...
def user_productivity_detail(request, user_id):
    ...
    productivity_records = UserProductivity.objects.filter(user=user).order_by("-date")[
        :14
    ]
...
def analytics_snapshots(request):
    ...
    global_snapshots = AnalyticsSnapshot.objects.filter(project=None).order_by(
        "-timestamp"
    )
    project_snapshots = AnalyticsSnapshot.objects.exclude(project=None).order_by(
        "-timestamp"
    )
...
def task_analytics(request):
    ...
    completion_trend = (
        Task.objects.filter(status=Task.COMPLETED, completed_at__gte=six_months_ago)
...
def generate_report(request, report_id):
    ...
    messages.success(
        request,
        f"Report generation has been started. You'll be notified when it's ready.",
    )
...
def tasks_by_status_chart(request):
    ...
    return JsonResponse(
        {"labels": status_labels, "data": [item["count"] for item in task_status]}
    )
...
def tasks_by_priority_chart(request):
    task_priority = list(
        Task.objects.values("priority").annotate(count=Count("id")).order_by("priority")
    )
    ...
    return JsonResponse(
        {"labels": priority_labels, "data": [item["count"] for item in task_priority]}
    )
...
def completion_time_trend_chart(request):
    ...
    try:
        ...
        completion_trend = list(
            TaskCompletionTime.objects.filter(completion_date__gte=twelve_weeks_ago)
            .annotate(
            ...
            .values("week")
            .annotate(avg_days=Avg("days_to_complete"))
            .order_by("week")
        )
        ...
        if completion_trend:
            return JsonResponse(
                {
                    "labels": [
                        item["week"].strftime("%Y-%m-%d") for item in completion_trend
                    ],
                    "data": [
                        round(item["avg_days"], 1) if item["avg_days"] else 0
        ...
        sample_data.append(
            {
                ...
                "avg_days": round(
                    2.0 + (i % 3) * 0.5, 1
                ),  # Vary between 2.0, 2.5, and 3.0 days
            }
        )
...
def user_productivity_chart(request):
    ...
    if user_id:
        user = get_object_or_404(User, id=user_id)
        productivity_data = list(
            UserProductivity.objects.filter(user=user).order_by("date")[:30]
        )

        return JsonResponse(
            {
                "labels": [
                    item.date.strftime("%Y-%m-%d") for item in productivity_data
                ],
...
def project_progress_chart(request):
    ...
    if project_id:
        ...
        try:
            snapshots = list(
                AnalyticsSnapshot.objects.filter(project=project).order_by("timestamp")[
                    :30
                ]
            )
        ...
        for day_offset in range(30):
            ...
            total_count = Task.objects.filter(
                project=project, created_at__lt=next_day
            ).count()

            # Get completed tasks for this project completed by this day
            completed_count = Task.objects.filter(
                project=project, status=Task.COMPLETED, completed_at__lt=next_day
            ).count()

            if total_count > 0:
                tasks_by_day.append(
                    {"date": day, "total": total_count, "completed": completed_count}
                )
