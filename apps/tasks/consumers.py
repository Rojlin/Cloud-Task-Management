...
class TaskCommentsConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if task_data:
            await self.send(
                text_data=json.dumps({"type": "initial_data", "task": task_data})
            )
                ...
                self.task_group_name,
                {
                    ...
                    "comment": {
                        ...
                        "user": {
                            ...
                            "full_name": comment_obj.user.get_full_name()
                            or comment_obj.user.username,
                        },
                    },
                },
            )
    ...
    def get_task_data(self):
        ...
        try:
            ...
            attachments = TaskAttachment.objects.filter(task=task).order_by(
                "uploaded_at"
            )
            ...
            attachments_data = [
                {
                    ...
                    "user": {
                        ...
                        "full_name": attachment.user.get_full_name()
                        or attachment.user.username,
                    },
                }
            ...
            return {
                ...
                "created_by": {
                    ...
                    "full_name": task.created_by.get_full_name()
    ...
    def save_comment(self, comment_text):
        ...
        comment = TaskComment.objects.create(
            task=task, user=self.user, comment=comment_text
        )
        return comment
    ...
    def create_status_update_notification(self, status):
        ...
        if (
            task.assigned_to
            and task.assigned_to != task.created_by
            and task.assigned_to != self.user
        ):
            status_display = dict(Task.STATUS_CHOICES).get(status, status)
            Notification.objects.create(
                user=task.assigned_to,
