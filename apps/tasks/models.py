    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            if self.status != self.COMPLETED:
                ...
                days_threshold = 3  # Configure the days threshold for approaching deadlines
                
                # If due date is within threshold and has been changed
                if (self.due_date != old_task.due_date and 
                    self.due_date > today and 
                    (self.due_date - today).days <= days_threshold):
...
class TaskComment(models.Model):
    ...
    comment = models.TextField()  # Changed from content to comment to match database schema
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
