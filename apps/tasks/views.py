...
class TaskAccessMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        ...
        return ProjectMember.objects.filter(project=task.project, user=self.request.user).exists()
...
class TaskListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['overdue_tasks'] = tasks.filter(due_date__lt=timezone.now().date()).exclude(status=Task.COMPLETED).count()
...
class TaskCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        project_manager = ProjectMember.objects.filter(
            project=project, 
            role=ProjectMember.PROJECT_MANAGER
        ).first()
...
class TaskDetailView(LoginRequiredMixin, TaskAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not can_edit:
            project_member = ProjectMember.objects.filter(project=task.project, user=user).first()
            if project_member and project_member.role in [ProjectMember.PROJECT_MANAGER, ProjectMember.LEADER]:
                can_edit = True
...
class TaskUpdateView(LoginRequiredMixin, TaskAccessMixin, UpdateView):
    ...
    def test_func(self):
        ...
        project_member = ProjectMember.objects.filter(project=task.project, user=user).first()
        return project_member and project_member.role in [ProjectMember.PROJECT_MANAGER, ProjectMember.LEADER]
    ...
    def form_valid(self, form):
        ...
        if form.instance.assigned_to and old_task.assigned_to != form.instance.assigned_to:
        ...
        if changes:
            ...
            messages.success(self.request, f'Task "{form.instance.title}" updated successfully. Changed: {change_msg}.')
        else:
...
class TaskPriorityUpdateView(LoginRequiredMixin, TaskAccessMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        messages.success(request, f'Task priority updated to {task.get_priority_display()}.')
...
class ProjectTaskKanbanView(LoginRequiredMixin, TaskAccessMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'in_progress_tasks': Task.objects.filter(project=project, status=Task.IN_PROGRESS),
            'review_tasks': Task.objects.filter(project=project, status=Task.REVIEW),
            'completed_tasks': Task.objects.filter(project=project, status=Task.COMPLETED),
...
def update_task_status(request):
    try:
        ...
        if (not request.user.is_admin and 
            request.user != task.created_by and 
...
def get_tasks_json(request):
    ...
    if project_id:
        ...
        if (not user.is_admin and 
            user != project.created_by and 
            not ProjectMember.objects.filter(project=project, user=user).exists()):
            return JsonResponse({'status': 'error', 'message': 'Permission denied'}, status=403)
    ...
    for task in tasks:
        ...
        if task.status == Task.COMPLETED:
            ...
            color = '#dc3545' if task.is_overdue else '#6c757d'  # Red for overdue, gray for todo
