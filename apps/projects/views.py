...
from .forms import ProjectForm, ProjectMemberForm, ProjectFilterForm, ProjectMemberInviteForm
...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['tasks_high_priority'] = tasks.filter(priority__in=[Task.HIGH, Task.URGENT]).count()
        ...
        context['completed_projects'] = projects.filter(status=Project.COMPLETED).count()
        ...
        context['overdue_projects'] = projects.filter(
            end_date__lt=timezone.now().date()
        ).exclude(status__in=[Project.COMPLETED, Project.CANCELLED]).count()
...
class ProjectAccessMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        ...
        return ProjectMember.objects.filter(project=project, user=self.request.user).exists()
...
class ProjectListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_projects'] = projects.count()
        context['active_projects'] = projects.filter(status__in=[Project.PLANNING, Project.IN_PROGRESS]).count()
        context['completed_projects'] = projects.filter(status=Project.COMPLETED).count()
        context['overdue_projects'] = projects.filter(end_date__lt=timezone.now().date()).exclude(status__in=[Project.COMPLETED, Project.CANCELLED]).count()
...
class ProjectDetailView(LoginRequiredMixin, ProjectAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        task_counts = Task.objects.filter(project=project).values('status').annotate(count=Count('status'))
...
class ProjectUpdateView(LoginRequiredMixin, ProjectAccessMixin, UpdateView):
    ...
    def test_func(self):
        ...
        return member and member.role in [ProjectMember.PROJECT_MANAGER, ProjectMember.LEADER]
...
class ProjectDeleteView(LoginRequiredMixin, ProjectAccessMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        project = self.get_object()
        messages.success(self.request, f'Project "{project.name}" deleted successfully.')
...
class ProjectMemberCreateView(LoginRequiredMixin, ProjectAccessMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f'{form.instance.user.username} added to project successfully.')
        return response
    
    def get_success_url(self):
        return reverse('projects:members', kwargs={'project_id': self.kwargs['project_id']})
...
class ProjectMemberUpdateView(LoginRequiredMixin, ProjectAccessMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f'{form.instance.user.username}\'s role updated successfully.')
        return response
...
class ProjectMemberDeleteView(LoginRequiredMixin, ProjectAccessMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(self.request, f'{member.user.username} removed from project successfully.')
        return super().delete(request, *args, **kwargs)
...
class ProjectMemberInviteView(LoginRequiredMixin, ProjectAccessMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if invited_users:
                messages.success(request, f'Successfully added {len(invited_users)} members to the project.')
            
            if existing_users:
                messages.warning(request, f'The following users are already members: {", ".join(existing_users)}')
...
class ProjectMemberApiView(LoginRequiredMixin, View):
    ...
    def get(self, request, project_id):
        try:
            ...
            if not (request.user.is_admin or project.created_by == request.user or is_member):
...
class ProjectStatisticsView(LoginRequiredMixin, ProjectAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Format for chart.js
        status_labels = [dict(Task.STATUS_CHOICES)[status] for status in [Task.TODO, Task.IN_PROGRESS, Task.REVIEW, Task.COMPLETED]]
        status_data = [
        ...
        assignee_tasks = Task.objects.filter(project=project).values('assigned_to__username').annotate(count=Count('assigned_to'))
        assignee_labels = [item['assigned_to__username'] or 'Unassigned' for item in assignee_tasks]
        ...
        date_range = [(thirty_days_ago + timezone.timedelta(days=i)).isoformat() for i in range(31)]
