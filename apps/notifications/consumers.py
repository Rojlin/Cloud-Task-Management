...
class NotificationConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        # Debug information
        print(f"Adding user {self.user.id} ({self.user.username}) to notification group: {self.notification_group_name}")
    ...
    async def notification_message(self, event):
        ...
        print(f"NotificationConsumer received notification_message event for user {self.user.id}")
...
class TaskConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        # If not authenticated, send a message indicating they need to authenticate
        if not self.user.is_authenticated:
            print(f"User not authenticated for task connection, sending auth required message")
            await self.send(text_data=json.dumps({
                'error': 'Authentication required',
                'authenticated': False
            }))
