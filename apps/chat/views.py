...
class ChatRoomCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f'Chat room "{self.object.name}" created successfully.'
        )
        return response
...
class ChatRoomUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        messages.success(
            self.request, f'Chat room "{self.object.name}" updated successfully.'
        )
        return response
...
class ChatRoomAddMembersView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user_ids = request.POST.getlist(
            "members"
        )  # Changed from 'users' to match the form
        ...
        if added_users:
            ...
            usernames = ", ".join(
                [user.get_full_name() or user.username for user in added_users]
            )
        ...
        messages.success(
            request, f'Members added to chat room "{room.name}" successfully.'
        )
